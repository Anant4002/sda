...
function buildSidebar() {
    return `
        <div class="section">
            <div class="section-title">Sub-orbital Threat Detection</div>
            <div class="micro-card">
                Rapid identification of ballistic missiles, hypersonic vehicles, and re-entry objects from direct sensor tracks.
            </div>
        </div>
        <div class="section">
            <div class="section-title">Live Sensor Simulation</div>
            <div class="micro-card" style="margin-bottom: 8px;">
                Inject simulated radar tracks to test trajectory fitting and classification.
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1px 1fr; gap: 8px;">
                <button id="simMissileButton" class="danger btn-sm">Sim Missile</button>
                <div style="background: rgba(255,255,255,0.1);"></div>
                <button id="simHGVButton" class="warning btn-sm">Sim HGV</button>
            </div>
            <button id="simMeteorButton" class="secondary btn-sm" style="width:100%; margin-top: 8px;">Sim Meteor</button>
        </div>
        <div class="section">
...
export default {
    id: "rapid-processing",
    label: "Rapid Processing & Response",
    eyebrow: "Sub-orbital Identification",
    description: "Low-latency trajectory fitting and classification for missiles, HGVs, and re-entry objects.",
    dockEyebrow: "Defense",
    dockLabel: "Rapid Response",
    status: "ready",
    mount(ctx) {
        ...
        const injectSimulatedTrack = async (type) => {
            setStatus(`Simulating ${type} sensor track...`);
            ...
            for (let i = 0; i < 5; i++) {
                points.push({
                    ...
                    z: (150 - i * 5) * 100,
                    altKm: type === "Meteor" ? 200 - i * 10 : 80 - i * 2,
                    lat: startLat + i * 0.1,
                    lon: startLon + i * 0.1,
        ...
        scope.add(document.getElementById("simMissileButton"), "click", () => injectSimulatedTrack("Missile"));
        scope.add(document.getElementById("simHGVButton"), "click", () => injectSimulatedTrack("HGV"));
        ...
        refreshTracks();
        setStatus("Rapid Processing module ready for sub-orbital monitoring.");
